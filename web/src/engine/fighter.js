...
import { hitstun as hitstunFor, velocity as launchVelocity } from './knockback.js';
...
export class Fighter {
  ...
  _applyLaunch() {
    const v = this.pendingLaunch;
    this.pendingLaunch = null;
    this.vx = v.vx; this.vy = v.vy;
    if (v.vy > 0.5 || !this.onGround) { this.onGround = false; this.platform = null; }
  }
  ...
  _stepAir(ctx) {
    ...
    if (this.consume('jump') && this.jumpsLeft > 0) {
  ...
  applyBlock({ damage, facing, perfect }) {
    const { blockstun } = this._knock();
    this.blockstun = perfect ? Math.floor(blockstun(damage) / 2) : blockstun(damage);
    if (!perfect) this.shield -= damage * SHIELD.damageMul;
    this.vx = facing * (damage / 5) * 10;
    if (this.shield <= 0) this.breakShield();
  }
  _knock() { return { blockstun: (d) => 4 + Math.floor(d * 0.8) }; }
  ...
  applyHit(hit) {
    ...
    const v = launchVelocity(launch, hit.angle, hit.facing, this.di);
    if (this.onGround && v.vy < 0) v.vy = -v.vy * 0.8;
    if (hit.pull) { this.x += -hit.facing * Math.min(hit.pull, Math.abs(this.x - (hit.attackerX ?? this.x))); }
    this.pendingLaunch = v;
  ...
  _physics(ctx) {
    ...
    if (res.wall) { if (this.state === 'hitstun' && Math.abs(this.vx) > 40) { this.vx = -this.vx * 0.5; } else this.vx = 0; }
  ...
  _land(p, ctx) {
    ...
    if (s === 'hitstun' || (s === 'air' && this.tumbling)) {
      if (this.tumbling) {
        if (this.buffer.guard > 0 && this.buffer.guard >= INPUT_BUFFER - TECH.window) {
          this.buffer.guard = 0; this.tumbling = false;
