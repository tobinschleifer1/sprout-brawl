...
export class Match {
  ...
  step() {
    ...
    for (const f of this.fighters) {
      ...
      if (!f.alive && f.koTimer > 0) {
        f.koTimer--;
        if (f.koTimer === 0 && (this.training || f.stocksLeft() > 0)) this.respawn(f);
      }
    }
  ...
  knockOut(f) {
    ...
    if (this.teams) this.teamPool[f.team] = Math.max(0, this.teamPool[f.team] - 1); else f.stocks = Math.max(0, f.stocks - 1);
  ...
  finish() {
    ...
    this.results = { rows, winnerTeam, teams: this.teams, timed: this.timed, mode: this.mode };
